...
`src/nav.zsh:8` and `src/nav.py:32`): `config.toml`, `cwd`, `sub/<tty>.fifo`. Deleting that
directory returns the project to first-run behaviour.
...
- **TTY basename symmetry.** `_nav_tty()` (`src/nav.zsh:14`) and `self_tty()`
  (`src/nav.py:152`) must produce byte-identical strings — that string is both the FIFO stem
  and the self-exclusion key in `publish()`. Diverge and the driving terminal silently
  fights its own `cd`.
